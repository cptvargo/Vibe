...
export function SimilarArtists({ artist, onArtistSelect, accent = '#7c3aed', filterIds = null }) {
  const [similar, setSimilar] = useState([]);

  useEffect(() => {
    if (!artist?.Id) return;
    getSimilarArtists(artist.Id).then(results => {
      const filtered = filterIds ? results.filter(a => filterIds.has(a.Id)) : results;
      setSimilar(filtered);
    });
  }, [artist?.Id]);
          ...
          <div key={a.Id} onClick={() => onArtistSelect?.({ Id: a.Id, Name: a.Name })} style={{ flexShrink: 0, width: 80, cursor: 'pointer', textAlign: 'center' }}>
            <div style={{ width: 80, height: 80, borderRadius: '50%', overflow: 'hidden', background: '#1e1e2e', marginBottom: 8, boxShadow: '0 4px 16px rgba(0,0,0,0.4)' }}>
              <img
                ...
                onError={(e) => { e.target.src = `${import.meta.env.BASE_URL}artists/${a.Name}.png`; e.target.onerror = () => { e.target.style.display = 'none'; }; }}
              />
            </div>
